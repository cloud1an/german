...
- `profile.js` — Yerel profil sistemi (şifresiz, kullanıcı adı bazlı). `Profile.namespace(key)` ile localStorage anahtarlarına kullanıcı adı eklenir. İlk kez kullanıcı ayarlandığında `migrateBareKeysOnce` eski isimsiz verileri yeni kullanıcıya taşır.
